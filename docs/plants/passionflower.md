---
layout: default
title: Passionflower (Maypop)
---
<div class="plant-hero">
  <a href="/FloridaLiving/assets/passionflower.jpg" target="_blank">
    <img src="/FloridaLiving/assets/passionflower.jpg" alt="Passionflower Vine">
  </a>

  <div class="plant-facts">
    <h3>Quick Facts</h3>
    <ul>
      <li><strong>USDA zones:</strong> 5–10 (native Florida wildflower!)</li>
      <li><strong>Sun:</strong> Full sun to part shade</li>
      <li><strong>Water:</strong> Average to moist</li>
      <li><strong>Height:</strong> 6–15 ft vine (climbs fence)</li>
      <li><strong>Bloom time:</strong> Summer–fall</li>
    </ul>
    <p><strong>Primary Use:</strong> Butterfly host → only food for Gulf fritillary in Florida</p>
  </div>
</div>

<div class="full-width">
  <h3>Wildlife Supported</h3>
  <ul>
    <li>Only host for Gulf fritillary caterpillars</li>
    <li>Nectar for bees, butterflies, hummingbirds</li>
  </ul>

  <h3>Pests to Watch</h3>
  <ul>
    <li>Gulf fritillary caterpillars → **DO NOT KILL** — they’re the whole point!</li>
  </ul>

  <h3>Unusual / Fun Facts</h3>
  <ul>
    <li>Fruit (maypop) is edible when yellow — tastes like tropical bubblegum</li>
    <li>Used by Native Americans as a calming tea</li>
  </ul>

  <h3>Local Care Tips (The Villages 9b)</h3>
  <ul>
    <li>Let it climb the fence — no trellis needed</li>
    <li>Dies back in winter, explodes in spring</li>
    <li>Current status: Pos 3 — fertilized, foiled, mulched Nov 20</li>
  </ul>
</div>

<h3>Growth Over Time</h3>
<table class="log-table table-growth">
  <tr><th>Date</th><th>Photo</th><th>Notes</th></tr>
  <tr>
    <td><strong>Nov 20, 2025</strong></td>
    <td><a href="/FloridaLiving/assets/passionflower.jpg" target="_blank">
      <img src="/FloridaLiving/assets/passionflower.jpg" width="100" alt="Nov 20, 2025">
    </a></td>
    <td>Fertilized, foiled, mulched — ready for winter</td>
  </tr>
</table>

<h3>Harvest Log</h3>
<table class="log-table table-harvest">
  <tr><th>Date</th><th>Fruit Count</th><th>Notes</th></tr>
</table>

<h3>Pests Encountered</h3>
<table class="log-table table-pests">
  <tr><th>Date</th><th>Pest</th><th>Treatment</th></tr>
</table>

<h3>Notes from the Gardener</h3>
<table class="log-table table-notes">
  <tr><th>Date</th><th>Note</th></tr>
  <tr><td>Nov 20, 2025</td><td>The cats are already trying to eat the caterpillars — training in progress!</td></tr>
</table>

<hr>
<p style="text-align:center;">
  <a href="/FloridaLiving/plants/pineapple.html">← Previous: Pineapple</a> • 
  <a href="/FloridaLiving/plants/lime.html">Next: Lime Tree →</a>
  <br><br>
  <a href="/FloridaLiving/plants.html">← All Plants</a> • 
  <a href="/FloridaLiving/backyard.html">Backyard Overview</a>
</p>
<hr>OLDFILE<hr>
# Passionflower (Maypop)
**Passiflora incarnata** | Passifloraceae

<img src = "https://raw.githubusercontent.com/ksmomof2-home/FloridaLiving/main/docs/assets/passionflower.jpg?" width=400 alt = "Maypop flower">

### Quick Facts
- USDA zones: 5–10 (native Florida wildflower!)
- Sun: Full sun to part shade
- Water: Average to moist
- Mature height in pot: 6–15 ft vine (climbs fence)
- Days to bloom: 60–90 from transplant

### Primary Use
- Butterfly host → **only food for Gulf fritillary caterpillars in Florida**

### Wildlife Supported
- Host for: Gulf fritillary (orange spiky cats!)
- Nectar for: bees, butterflies, hummingbirds

### Pests to Watch
- Gulf fritillary caterpillars → **DO NOT KILL** — they are the point!
- Whiteflies → ignore passionflower

### Unusual / Fun Facts
- The fruit (maypop) is edible when yellow and wrinkly — tastes like tropical bubblegum
- Used by Native Americans as a calming tea

### Local Care Tips (The Villages 9b)
- Let it climb the fence — no trellis needed
- Dies back in winter, explodes in spring
- Current status: Pos 3 — fertilized, foiled, mulched Nov 20

QR code on stake → https://yourusername.github.io/garden/passionflower
