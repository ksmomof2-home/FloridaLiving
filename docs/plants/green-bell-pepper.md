---
layout: default
title: Green Bell Pepper
---
<div class="plant-hero">
  <a href="/FloridaLiving/assets/emptypot.jpg" target="_blank">
    <img src="/FloridaLiving/assets/emptypot.jpg" alt="Green Bell Pepper">
  </a>

  <div class="plant-facts">
    <h3>Quick Facts</h3>
    <ul>
      <li><strong>USDA zones:</strong> All (annual)</li>
      <li><strong>Sun:</strong> Full sun</li>
      <li><strong>Water:</strong> Consistent</li>
      <li><strong>Height:</strong> 2–3 ft</li>
      <li><strong>Fruit:</strong> 70–80 days</li>
    </ul>
    <p><strong>Primary Use:</strong> Culinary → salads, stuffing, fajitas</p>
  </div>
</div>

<div class="full-width">
  <h3>Wildlife Supported</h3>
  <ul>
    <li>Nectar for bees</li>
  </ul>

  <h3>Pests to Watch</h3>
  <ul>
    <li>Whiteflies → foil + soap spray</li>
  </ul>

  <h3>Unusual / Fun Facts</h3>
  <ul>
    <li>Turns red/yellow if left on the plant — sweeter!</li>
  </ul>

  <h3>Local Care Tips (The Villages 9b)</h3>
  <ul>
    <li>Pos 9 (after Feb bush move)</li>
  </ul>
</div>

<h3>Growth Over Time</h3>
<table class="log-table table-growth">
  <tr><th>Date</th><th>Photo</th><th>Notes</th></tr>
</table>

<h3>Harvest Log</h3>
<table class="log-table table-harvest">
  <tr><th>Date</th><th>Count</th><th>Notes</th></tr>
</table>

<h3>Pests Encountered</h3>
<table class="log-table table-pests">
  <tr><th>Date</th><th>Pest</th><th>Treatment</th></tr>
</table>

<h3>Notes from the Gardener</h3>
<table class="log-table table-notes">
  <tr><th>Date</th><th>Note</th></tr>
</table>

<hr>
<p style="text-align:center;">
  <a href="/FloridaLiving/plants/jalapeno-cilantro.html">← Previous: Jalapeño + Cilantro</a> • 
  <a href="/FloridaLiving/plants/poblano.html">Next: Poblano Pepper →</a>
  <br><br>
  <a href="/FloridaLiving/plants.html">← All Plants</a> • 
  <a href="/FloridaLiving/backyard.html">Backyard Overview</a>
</p>
